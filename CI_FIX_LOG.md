...
## ClawBase 3.0.1/build 2 installability gate

- Device feedback after the restored bundle metadata still showed the full Replica IPA could not be installed, even though the signed CI path was green.
- A critical compatibility fact was then isolated: the installed ClawTalk 3.0.0 and the WEIXIN diagnostic use the same Host bundle ID `app.lgm.7517`, Keyboard bundle ID `app.lgm.7517.123`, Team `X5G6AN3DYX`, and App Group `group.7518554`. The previous Replica package used version 1.0, so the install attempt could be treated as a downgrade/replacement conflict.
- Phase 1 now uses an isolated two-target `ClawBase` XcodeGen project at marketing version 3.0.1, build 2. It intentionally contains only a minimal Host and keyboard shell; V14 UI, engine, panels, and system extensions remain outside the package until a real-device install passes.
- Added `Tools/verify_claw_base_ci.py` to statically enforce IDs, version/build, keyboard extension metadata, App Group, Phase 1 isolation, dual-profile signing order, minimal entitlements, and workflow secret references.
- Added `ClawBase/ci_build_unsigned.sh` to build and validate the exact 3.0.1/build 2 Host + Keyboard structure before any signing step.
- Added `ClawBase/ci_sign_and_validate.sh` to reuse the proven exact-profile/minimal-entitlement pattern, sign Keyboard first and Host last, validate post-sign entitlements and codesign, and emit `ClawBase-3.0.1-2-signed.ipa` with SHA-256 evidence.
- Added `.github/workflows/ios-claw-base-signed-ci.yml`, restricted to `work/v14-signed-device`, with signing material confined to an ephemeral keychain/directory and successful IPA upload only after all gates pass.
- Phase 1 is not complete until the untouched CI IPA installs over ClawTalk 3.0.0 and Host + Keyboard pass real-device checks.
