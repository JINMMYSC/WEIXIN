...
public struct WTDeviceSyncPanelView: View {
        ...
        VStack(spacing: 8) {
            HStack {
                ...
                Button("更换") { runtime.regenerateTransferPairingCode() }
                    .font(.system(size: 11))
                    .buttonStyle(.plain)
                    .foregroundStyle(WTThemeColor353.accent)
            }
            HStack(spacing: 5) {
            ...
            Text("两台设备使用相同配对码后，再在附近设备中选择目标设备。")
                .font(.system(size: 10))
                .foregroundStyle(WTThemeColor353.secondaryText)
    ...
    private var transferStatusCard: some View {
        ...
        case .idle:
            EmptyView()
        case .discovering:
            statusRow(title: "正在查找附近设备", detail: "保持两台设备在同一局域网", progress: nil)
        case .connecting(let peer):
            statusRow(title: "正在连接 \(peer)", detail: "正在校验配对信息", progress: nil)
        case .transferring(let peer, let progress):
            statusRow(title: "正在与 \(peer) 传输", detail: "\(Int((progress * 100).rounded()))%", progress: progress)
        case .completed(let peer):
            statusRow(title: "传输完成", detail: peer, progress: 1)
        case .failed(let reason):
            statusRow(title: "传输失败", detail: reason, progress: nil)
        }
    }
    ...
    private var trustedSection: some View {
        if !runtime.trustedTransferDevices.isEmpty {
            VStack(spacing: 1) {
                HStack {
                    Text("已配对设备").font(.system(size: 11)).foregroundStyle(WTThemeColor353.secondaryText)
                    Spacer()
                }
                ...
                ForEach(runtime.trustedTransferDevices) { device in
                    HStack(spacing: 12) {
                        ...
                        Spacer()
                        Button("移除") { runtime.revokeTrustedTransferDevice(device.id) }
                            .font(.system(size: 11))
                            .buttonStyle(.plain)
                            .foregroundStyle(WTThemeColor353.secondaryText)
                    }
    ...
    private var peerSection: some View {
        if runtime.devicePeers.isEmpty {
            WTEmptyPanelState(systemName: "laptopcomputer.and.iphone", title: "正在查找附近设备", subtitle: "在另一台设备打开微信输入法，并保持在同一局域网。")
                .frame(minHeight: 104)
        } else {
            VStack(spacing: 1) {
                HStack {
                    Text("附近设备").font(.system(size: 11)).foregroundStyle(WTThemeColor353.secondaryText)
                    Spacer()
                }
                .padding(.horizontal, 2)
