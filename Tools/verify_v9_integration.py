...
# Corpus scope.
corpus=json.loads((root/'ReverseEngineering/V9/IME_BEHAVIOR_CORPUS.json').read_text())
...
visual=json.loads((root/'ReverseEngineering/V9/VISUAL_CAPTURE_MANIFEST.json').read_text())
if len(visual.get('cases',[])) < 50: errors.append('visual manifest has fewer than 50 cases')
# Xcode shared source.
yml=(root/'XcodeIntegration/project.yml').read_text()
