...
def main() -> int:
    ...
    require("MARKETING_VERSION: 3.0.1" in project, "ClawBase marketing version must be 3.0.1")
    require("CURRENT_PROJECT_VERSION: 2" in project, "ClawBase build must be 2")
    ...
    forbidden_phase2_paths = (
        ...
        "iOSServices",
        "HamsterBridge",
        "WTKeyboardInputViewController",
    )
    for forbidden in forbidden_phase2_paths:
        require(forbidden not in project, f"Phase 2 dependency leaked into ClawBase project: {forbidden}")
    ...
    require("work/v14-signed-device" in workflow, "ClawBase workflow must stay on signed-device branch")
    ...
    print("ClawBase CI configuration verifier passed")
    return 0
...
if __name__ == "__main__":
    try:
        ...
        print(f"ClawBase CI configuration verifier failed: {error}", file=sys.stderr)
        raise SystemExit(1)
